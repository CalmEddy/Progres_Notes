...
import { Collection, SearchCriteria } from '@/lib/collections/types';
import BinderTree from './BinderTree';
import BinderSearch, { SearchType } from './BinderSearch';
...
interface BinderSidebarProps {
  ...
  loading: boolean;
  error: string | null;
  onSaveSearchAsCollection?: (searchCriteria: SearchCriteria) => void;
  chunkSearchResults?: BinderSearchResult[] | null;
  onClearChunkSearchResults?: () => void;
...
export default function BinderSidebar({
  ...
  loading,
  error,
  onSaveSearchAsCollection,
  chunkSearchResults,
  onClearChunkSearchResults,
  isChunkSearching = false,
}: BinderSidebarProps) {
  const [searchQuery, setSearchQuery] = useState('');
  const [searchType, setSearchType] = useState<SearchType>('auto');
  const [searchFilters, setSearchFilters] = useState<any>({});
  const [searchThreshold, setSearchThreshold] = useState<number>(0.7);
  const [searchResults, setSearchResults] = useState<BinderSearchResult[] | null>(null);
  ...
  useEffect(() => {
    ...
    // Clear chunk search results when starting a text search
    if (onClearChunkSearchResults && chunkSearchResults !== null && chunkSearchResults !== undefined) {
      onClearChunkSearchResults();
    }
    ...
    searchTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        const response = await fetch('/api/binder/search', {
          method: 'POST',
          headers,
          body: JSON.stringify({
            query: searchQuery,
        ...
        if (response.ok) {
          ...
          if (data.results?.length === 0 && (searchType === 'semantic' || searchType === 'combined')) {
            console.warn('[Client] ⚠️ No semantic search results. Check your Next.js SERVER TERMINAL for detailed diagnostics.');
            console.warn('[Client] To run a diagnostic test, open: http://localhost:3000/api/binder/search/test');
            console.warn('[Client] Server logs to check:', {
              query: searchQuery,
              threshold: searchThreshold,
              searchType,
              lookFor: [
                '🔍 [CHUNK SEARCH] Chunk check',
                '❌ [CHUNK SEARCH] RPC function error',
                '⚠️ [CHUNK SEARCH] No data returned',
                '✅ [SEMANTIC SEARCH] Successfully found chunk matches',
              ],
            });
          }
  ...
  }, [searchQuery, searchType, searchFilters, searchThreshold, chunkSearchResults, onClearChunkSearchResults]);
  ...
  const handleCreateCollection = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setCollections(prev => [...prev, newCollection].sort((a, b) => a.position - b.position));
        // Select the newly created collection
        onCollectionSelect(newCollection.id);
      } else {
    ...
    <div className="w-80 bg-white border-r border-gray-200 flex flex-col h-full overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-gray-200 flex-shrink-0">
        <BinderSearch
          ...
          onSaveAsCollection={(searchCriteria: SearchCriteria) => {
            if (onSaveSearchAsCollection) {
              onSaveSearchAsCollection(searchCriteria);
            }
          }}
        />
      </div>
      ...
      <div className="px-4 py-2 border-b border-gray-200 flex-shrink-0">
        <div className="flex items-center justify-between mb-2">
          ...
          >
            <svg
              className={`w-4 h-4 transition-transform ${collectionsExpanded ? 'rotate-90' : ''}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
        ...
        {collectionsExpanded && (
          <div className="space-y-1">
            {collectionsLoading ? (
              ...
              <div className="text-xs text-gray-500 py-2">No collections yet</div>
            ) : (
              collections.map((collection) => (
                <button
                  key={collection.id}
                  onClick={() => onCollectionSelect(collection.id)}
                  className={`w-full text-left px-3 py-1.5 text-sm rounded hover:bg-gray-100 transition-colors flex items-center justify-between gap-2 ${
        ...
        >
          {isSearching ? (
            ...
            <div className="flex items-center justify-center py-8">
              <div className="text-center">
                <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mb-2"></div>
                <p className="text-sm text-gray-500">Finding similar chunks...</p>
              </div>
            </div>
          ) : chunkSearchResults !== null && chunkSearchResults !== undefined ? (
            <>
              {/* Search results header with clear button */}
              <div className="sticky top-0 z-10 px-4 py-2 border-b border-gray-200 bg-blue-50 flex items-center justify-between flex-shrink-0">
                <div className="flex items-center gap-2">
                  <svg className="w-4 h-4 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                  </svg>
                  <span className="text-sm font-medium text-blue-900">
                    Similar Chunks ({chunkSearchResults.length})
                  </span>
                </div>
                {onClearChunkSearchResults && (
                  <button
                    onClick={onClearChunkSearchResults}
