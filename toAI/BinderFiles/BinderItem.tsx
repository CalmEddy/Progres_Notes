...
interface BinderItemProps {
  ...
  level: number;
  tags?: Tag[];
  isSearchResult?: boolean; // Indicates this item is from search results
...
export default function BinderItem({
  ...
  // Skip this check for search results - they're already filtered by the search
  const matchesSearch = searchQuery === '' || 
    item.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
    (item.type === 'note' && item.note?.body.toLowerCase().includes(searchQuery.toLowerCase()));

  // Don't filter out search results - they're already filtered by the search algorithm
  // Semantic search results might not contain the exact query text but are semantically similar
  if (!isSearchResult && !matchesSearch && !hasChildren) {
    return null;
  }
